# =============================================================================
# 文件功能：WSI Patch 切割脚本（旧版，保存图像像素）
# 与 create_patches_fp.py 的区别：
#   - patch 使用 createPatches_bag_hdf5（将图像像素保存到 h5，较耗磁盘）
#   - stitch 使用 StitchPatches（从已保存像素重建拼图）
#   - 额外支持 --custom_downsample 进一步对 patch 皮素组下采样（如 2x）
# 输出：save_dir/patches/*.h5 (images+coords), masks/*.png, stitches/*.png
# =============================================================================
...
def stitching(file_path, downscale = 64):
	"""从包含图像像素的 h5 文件重建 WSI 缩略预览图（调用 StitchPatches）"""
	start = time.time()
...
def segment(WSI_object, seg_params, filter_params):
	"""对 WSI 执行组织区域切割，返回 (WSI_object, 耗时)"""
	### Start Seg Timer
...
def patching(WSI_object, **kwargs):
	"""调用 createPatches_bag_hdf5 提取并保存含图像像素的 patch h5 文件（单进程，旧版接口）"""
	### Start Patch Timer
	start_time = time.time()
...
def seg_and_patch(source, save_dir, patch_save_dir, mask_save_dir, stitch_save_dir, 
	...
	"""
	主处理函数：逐个遍历 source 目录下所有 WSI 文件并按顺序执行：
	  seg -> save_mask -> patch -> stitch
	custom_downsample 支持对取出的 patch 进一步下采样（目前仅支持 1x 和 2x）。
	"""
