# =============================================================================
# 文件功能：WSI（全切片图像）处理核心类
# 主要功能：
#   - 支持 openslide（.svs/.tiff 等）和 opensdpc（.sdpc）双后端加载 WSI
#   - 组织切割（segmentTissue）：HSV → 中值模糊 → 阈値 → 轮廓过滤
#   - Patch 采样（process_contour / _getPatchGenerator）：基于轮廓的滞动窗口捡取
#     支持多进程并行处理（mp.Pool），自动过滤白底/黑底 patch
#   - 热图可视化（visHeatmap）：将注意力分数叠加到 WSI 图像上
#     支持 Gaussian 模糊、alpha 混合、表层组织遮罩
#   - 分块 Alpha 混合（block_blending）：避免大图直接加载内存溢出
# =============================================================================
...
import opensdpc
# 设置允许加载的最大图像像素数上限（防止超大 WSI 触发 PIL DecompressionBomb 错误）
...
#         self.name = ".".join(path.split("/")[-1].split('.')[:-1])
        ...
        self.contours_tissue = None   # 组织区域轮廓列表
        self.contours_tumor = None    # 肿瘾标注轮廓列表（XML/TXT 加载后填充）
        self.hdf5_file = None         # 已保存的 hdf5 patch 文件路径

    def getOpenSlide(self):
        return self.wsi

    def initXML(self, xml_path):
        """从 NDPA/QuPath XML 标注文件加载肿瘾轮廓，并按面积降序排序。"""
    ...
    def initTxt(self,annot_path):
        """从文本格式标注文件加载肿瘾轮廓，支持 Polygon 和多线段格式。"""
    ...
    def createPatches_bag_hdf5(self, save_path, patch_level=0, patch_size=256, step_size=256, save_coord=True, **kwargs):
        """
        使用旧式生成器方式（_getPatchGenerator）提取 patch 并保存到 HDF5 文件。
        注：新代码优先使用 process_contours + process_contour（多进程）方式。
        """
    ...
    def _getPatchGenerator(self, cont, cont_idx, patch_level, save_path, patch_size=256, step_size=256, custom_downsample=1,
        ...
        """
        基于单个组织轮廓的 patch 生成器（旧接口，单进程）。
        综合考虑了：自定义下采样、边界娹补、白底/黑底过滤、轮廓外点过滤。
        """
    ...
    def process_contours(self, save_path, patch_level=0, patch_size=256, step_size=256, **kwargs):
        """
        逐个处理所有组织轮廓，将周期内提取的 patch 坐标保存到同一个 hdf5 文件。
        内部调用 process_contour() 处理单个轮廓（多进程并行）。
        """
    ...
    def process_contour(self, cont, contour_holes, patch_level, save_path, patch_size = 256, step_size = 256,
        ...
        """
        对单个组织轮廓执行多进程 patch 坐标提取。
        流程：
          1. 计算轮廓边界框，限定扫描范围
          2. 生成所有候选坐标网格（meshgrid）
          3. 使用 mp.Pool 并行判断每个坐标是否在轮廓内
          4. 将有效坐标保存为 asset_dict['coords']
        top_left / bot_right 可限定 ROI 区域。
        """
        start_x, start_y, w, h = cv2.boundingRect(cont) if cont is not None else (0, 0, self.level_dim[patch_level][0], self.level_dim[patch_level][1])
    ...
    def visHeatmap(self, scores, coords, vis_level=-1, 
                   ...
                   cmap='coolwarm'):
        """
        将注意力分数渲染为热图并叠加到 WSI 图像上。
        主要步骤：
          1. 确定可视化层级和目标区域
          2. 将 patch 分数累加到 overlay 数组（处理重叠）
          3. （可选）百分位化 + Gaussian 模糊
          4. 使用 colormap 将分数映射为颜色
          5. 只在组织区域覆盖颜色（可选）
          6. Alpha 混合成感知图像
        Args:
    ...
    def block_blending(self, img, vis_level, top_left, bot_right, alpha=0.5, blank_canvas=False, block_size=1024):
        """
        分块执行 WSI 图像与热图的 alpha 混合。
        将大图分割为 block_size x block_size 的小块，逐块从原始 WSI 读取与之混合，
        避免一次性将整张 WSI 加入内存。
        """
        print('\ncomputing blend')
        downsample = self.level_downsamples[vis_level]
    ...
    def get_seg_mask(self, region_size, scale, use_holes=False, offset=(0,0)):
        """
        生成前景组织区域的二值掩码（bool 类型）。
        按轮廓面积降序绘制，保证大轮廓分配的组织标记不被小轮廓覆盖。
        use_holes=True 时进一步将空洞区域设为 0（不显示热图）。
        """
